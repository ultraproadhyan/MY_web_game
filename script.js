...
function renderVideos() {
    ...
    const filteredVideos = currentCategory === 'all' 
        ? videoData 
        : videoData.filter(video => video.category === currentCategory);
    ...
    loadMoreBtn.style.display = visibleVideos >= filteredVideos.length ? 'none' : 'block';
}
...
function setupEventListeners() {
    ...
    // Video cards (using event delegation)
    document.addEventListener('click', (e) => {
        const videoCard = e.target.closest('.featured-card, .video-card');
        if (videoCard) {
